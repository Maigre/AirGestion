		...
		function parse($sql) {
			...
			*/
			...
			if(count($queries) == 1 && !$union) {
				...
				{
					$queries['QuickAccess']['Type'] = 'INSERT';
					$queries['QuickAccess']['Table'] = str_replace("`","",$queries['INSERT']['table']);
				}
				else if (isset($queries['UPDATE'])) $queries['Type'] = 'UPDATE';
				else if (isset($queries['DELETE'])) $queries['Type'] = 'DELETE';
				else if (isset($queries['SELECT'])) $queries['Type'] = 'SELECT';
			}
			else $queries['Type'] = 'ERROR_PARSING';
